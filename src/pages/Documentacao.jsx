...
const SEL_STYLE = { width: '100%', background: '#000', border: '1px solid #333', color: '#fff', padding: '10px', fontFamily: 'Space Mono,monospace', fontSize: 12, outline: 'none' };
          ...
          <Section title="Documentos Anexados" action={canAdd && <Btn size="sm" onClick={() => setShowForm(f => !f)}>+ Anexar</Btn>}>
            ...
            {showForm && (
              <div style={{ background: '#0a0a0a', border: '1px solid #333', padding: 14, marginBottom: 10 }}>
                <div style={{ fontSize: 9, letterSpacing: 3, color: '#666', textTransform: 'uppercase', marginBottom: 8 }}>Novo Documento</div>
                <input
                  value={formName}
                  onChange={e => setFormName(e.target.value)}
                  placeholder="Nome do documento..."
                  style={{ background: '#000', border: '1px solid #333', color: '#fff', padding: '8px 10px', fontFamily: 'Space Mono,monospace', fontSize: 12, outline: 'none', width: '100%', boxSizing: 'border-box', marginBottom: 10 }}
                ...
                >
                  ...
                  <div style={{ fontSize: 8, color: '#444', marginTop: 3 }}>PDF · JPG · PNG · WORD · EXCEL</div>
                </div>
                <input
                  ref={fileRef}
                  type="file"
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={resetForm} style={{ flex: 1, padding: '8px', background: 'transparent', border: '1px solid #333', color: '#888', fontFamily: 'Space Mono,monospace', fontSize: 9, letterSpacing: 2, cursor: 'pointer', textTransform: 'uppercase' }}>
                    Cancelar
                  </button>
                  <button onClick={save} disabled={uploading || !formName.trim() || !selFile}
                    style={{ flex: 1, padding: '8px', background: uploading ? '#111' : '#fff', border: '1px solid #fff', color: uploading ? '#888' : '#000', fontFamily: 'Space Mono,monospace', fontSize: 9, letterSpacing: 2, cursor: uploading ? 'wait' : 'pointer', textTransform: 'uppercase' }}>
              ...
              list.map(doc => (
                <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#0a0a0a', border: '1px solid #1a1a1a', padding: 12, marginBottom: 4 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: '#666', marginTop: 1, letterSpacing: 1 }}>{doc.size} · {doc.date}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                    {doc.file && doc.file.startsWith('http') && (
                      <a href={doc.file} target="_blank" rel="noopener noreferrer"
                        style={{ fontSize: 9, letterSpacing: 1, padding: '4px 8px', border: '1px solid #333', color: '#888', fontFamily: 'Space Mono,monospace', textDecoration: 'none', display: 'inline-block' }}>
                        Ver
                      </a>
                    )}
                    <button onClick={() => handleDelete(doc)}
                      style={{ background: 'transparent', border: '1px solid #333', color: '#f44336', padding: '4px 8px', fontFamily: 'Space Mono,monospace', fontSize: 12, cursor: 'pointer' }}>
                      🗑️
                    </button>
                  </div>
                </div>
