...
const thS = { fontSize:8, letterSpacing:2, color:'#555', textTransform:'uppercase', padding:'7px 10px', borderBottom:'1px solid #1a1a1a', textAlign:'left', background:'#050505', whiteSpace:'nowrap' };
const tdS = { fontSize:11, padding:'8px 10px', borderBottom:'1px solid #0d0d0d' };
const selS = { background:'#000', border:'1px solid #333', color:'#fff', padding:'6px 8px', fontFamily:'Space Mono,monospace', fontSize:10, outline:'none' };

// ─── Sub-componentes ─────────────────────────────────────────────────────────
function KPI({ label, value, sub, color='#fff' }) {
  return (
    <div style={{ background:'#0a0a0a', border:'1px solid #1a1a1a', padding:'12px 10px' }}>
      <div style={{ fontSize:8, letterSpacing:3, color:'#555', textTransform:'uppercase', marginBottom:5 }}>{label}</div>
...
function SHead({ title }) {
  return (
    <div style={{ fontSize:9, letterSpacing:4, color:'#666', textTransform:'uppercase', padding:'18px 16px 8px', borderTop:'1px solid #0d0d0d' }}>
      {title}
    </div>
  );
}
...
function BarChart({ data, labelKey, valueKey, fmtTip, color='#fff', height=140 }) {
  ...
  if (!data.length || data.every(d=>!d[valueKey]))
    return <div style={{ textAlign:'center', padding:'20px 0', color:'#444', fontSize:10 }}>Sem dados</div>;
      ...
      <div style={{ flex:1, minWidth:120 }}>
        {segs.map((seg, i) => (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:6, marginBottom:5,
            ...
            <span style={{ fontSize:9, color:'#ccc', flex:1 }}>{seg[nameKey]}</span>
            <span style={{ fontSize:9, color:'#666', whiteSpace:'nowrap' }}>{(seg.frac*100).toFixed(1)}%</span>
            <span style={{ fontSize:9, color:'#444', whiteSpace:'nowrap', marginLeft:4 }}>{fmt(seg[valueKey])}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Relatorios() {
  ...
  if (!loaded) return (
    <div style={{ textAlign:'center', padding:'60px 16px', color:'#555', fontSize:10, letterSpacing:3, textTransform:'uppercase' }}>
      Carregando dados...
    </div>
  );
    ...
    <div id="relatorios-root">
      ...
      <div style={{ padding:'12px 16px 0' }}>
        <div style={{ background:'#0a0a0a', border:'1px solid #1a1a1a', padding:14, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          <div>
            <div style={{ fontSize:8, letterSpacing:3, color:'#555', textTransform:'uppercase', marginBottom:4 }}>Previsão de Receita</div>
            <div style={{ fontSize:8, color:'#444', marginBottom:4 }}>Shows confirmados com data futura</div>
            <div style={{ fontSize:24, fontWeight:700, fontFamily:'Bebas Neue,sans-serif', color:'#fff' }}>{fmt(previsao)}</div>
          </div>
          <div>
            <div style={{ fontSize:8, letterSpacing:3, color:'#555', textTransform:'uppercase', marginBottom:4 }}>Meta Mensal</div>
            {editMeta ? (
              <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                <input type="number" value={metaInput} onChange={e=>setMetaInput(e.target.value)}
                  style={{ width:100, background:'#000', border:'1px solid #fff', color:'#fff', padding:'4px 8px', fontFamily:'Space Mono,monospace', fontSize:12, outline:'none' }} />
                <button onClick={() => {
                  const v = parseFloat(metaInput)||0;
                  setMeta(v); localStorage.setItem('md_meta_mensal',String(v)); setEditMeta(false);
                }} style={{ padding:'4px 10px', background:'#fff', color:'#000', border:'none', fontFamily:'Space Mono,monospace', fontSize:9, cursor:'pointer' }}>OK</button>
              </div>
            ) : (
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                ...
                {isMaster() && (
                  <button onClick={()=>{setMetaInput(String(meta));setEditMeta(true);}} className="no-print"
                    style={{ fontSize:8, padding:'3px 8px', background:'transparent', border:'1px solid #333', color:'#555', fontFamily:'Space Mono,monospace', cursor:'pointer' }}>
                    Editar
                  </button>
                )}
              </div>
            ...
            {meta>0 && (
              <div style={{ fontSize:9, color:previsao>=meta?'#4caf50':'#f44336', marginTop:3 }}>
      ...
      <div style={{ padding:'8px 16px 14px' }}>
        ...
        {custosByCat.length>0 && (
          <div style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', borderTop:'1px solid #111', marginTop:10 }}>
            <span style={{ fontSize:9, color:'#666', letterSpacing:2, textTransform:'uppercase' }}>Total Despesas</span>
      ...
      <div style={{ padding:'0 16px 16px', overflowX:'auto' }}>
        {sortedFin.length===0 ? (
          <div style={{ color:'#444', fontSize:10, textAlign:'center', padding:'20px 0' }}>Sem dados financeiros</div>
        ) : (
            ...
            <tbody>
              {sortedFin.map((f,i) => (
                <tr key={i} style={{ background:i%2===0?'#050505':'transparent' }}>
                  ...
                  <td style={{ ...tdS, textAlign:'right', color:'#666' }}>{fmtDate(f.date)}</td>
              ...
              <tr style={{ background:'#111', borderTop:'2px solid #222' }}>
                <td style={{ ...tdS, fontSize:9, letterSpacing:2, textTransform:'uppercase', color:'#888', fontWeight:700 }} colSpan={2}>Total</td>
      ...
      <div style={{ padding:'0 16px 16px', overflowX:'auto' }}>
        {equipeRows.length===0 ? (
          <div style={{ color:'#444', fontSize:10, textAlign:'center', padding:'20px 0' }}>Sem escalações registradas</div>
        ) : (
      ...
      <div style={{ padding:'0 16px 16px', overflowX:'auto' }}>
        {sortedCli.length===0 ? (
          <div style={{ color:'#444', fontSize:10, textAlign:'center', padding:'20px 0' }}>Sem dados no CRM</div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr>
      ...
      <div style={{ padding:'0 16px 24px', overflowX:'auto' }}>
        {crmConversao.length===0 ? (
          <div style={{ color:'#444', fontSize:10, textAlign:'center', padding:'20px 0' }}>Sem dados de conversão</div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr>
