    ...
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #222' }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            flex: 1, padding: '11px 4px', fontSize: 14, letterSpacing: 1.5, textTransform: 'uppercase',
            ...
            <div key={m.id} onClick={() => setDetail(m)} style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', padding: 12, marginBottom: 6, cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 14, color: '#aaa', marginTop: 1 }}>{m.email}</div>
                </div>
              </div>
              {m.sarpas && (
                <div style={{ marginTop: 6 }}>
                  <span style={{ fontSize: 14, letterSpacing: 1, padding: '3px 8px', border: '1px solid #4caf50', color: '#4caf50', textTransform: 'uppercase' }}>
                    Sarpas: {m.sarpas}
                  </span>
                </div>
              )}
      ...
      {tab === 'escalar' && (
        <Section title="Escalar para Show">
          <select value={selectedShow} onChange={e => setSelectedShow(e.target.value)}
            style={{ width: '100%', background: '#000', border: '1px solid #333', color: '#fff', padding: '8px 10px', fontFamily: 'Space Mono,monospace', fontSize: 16, outline: 'none', marginBottom: 14 }}>
          ...
          {showId && (
            <>
              <div style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', padding: 12, marginBottom: 10 }}>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6 }}>Equipe Escalada</div>
                {currentScaled.length === 0 ? <div style={{ fontSize: 14, color: '#aaa' }}>Nenhum membro escalado</div> : currentScaled.map((sc, i) => {
                  const m = members.find(m => m.id === sc.memberId);
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 8px', background: '#111', border: '1px solid #222', marginBottom: 3 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 14, color: '#888', fontStyle: 'italic' }}>{sc.role}</div>
                      </div>
                      <span onClick={() => removeFromShow(showId, i)} style={{ fontSize: 14, color: '#f44336', cursor: 'pointer', padding: '0 4px' }}>✕</span>
                    </div>
                  );
                })}
              ...
              <div style={{ fontSize: 14, letterSpacing: 4, color: '#888', textTransform: 'uppercase', marginBottom: 8 }}>Adicionar à Equipe</div>
                ...
                return (
                  <div key={m.id} onClick={() => setScaleModal({ showId, memberId: m.id, role: '' })}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 14, color: busy ? '#f44336' : '#4caf50', letterSpacing: 1 }}>{busy ? 'Ocupado em outro show' : 'Disponível'}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div key={m.id} style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', padding: 12, marginBottom: 8 }}>
              ...
              {Object.keys(PERM_LABELS).map(k => (
                <div key={k} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0' }}>
                  <div style={{ fontSize: 14, color: '#888' }}>{PERM_LABELS[k]}</div>
      ...
      {modal === 'add' && (
        <Modal title={editingId ? 'Editar Membro' : 'Novo Membro'} onClose={() => { setModal(null); setEditingId(null); }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={{ fontSize: 14, letterSpacing: 3, color: '#aaa', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Grupo de Acesso</label>
          ...
          <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid #222' }}>
            <div style={{ fontSize: 14, letterSpacing: 3, color: '#aaa', textTransform: 'uppercase', marginBottom: 8 }}>Permissões</div>
            {Object.keys(PERM_LABELS).map(k => (
              <div key={k} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0' }}>
                <div style={{ fontSize: 14, color: '#888' }}>{PERM_LABELS[k]}</div>
        ...
        <Modal title="Detalhes do Membro" onClose={() => setDetail(null)}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              ...
              {detail.sarpas && <div style={{ fontSize: 14, color: '#4caf50' }}>Sarpas: {detail.sarpas}</div>}
            </div>
          </div>
      ...
      {scaleModal && (
        <Modal title="Escalar Membro" onClose={() => setScaleModal(null)}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{members.find(m => m.id === scaleModal.memberId)?.name}</div>
          <div style={{ fontSize: 14, color: '#888', marginBottom: 12 }}>{shows.find(s => s.id === scaleModal.showId)?.client}</div>
      ...
      {shareText && (
        <Modal title="Texto para Compartilhar" onClose={() => setShareText(null)}>
          <div style={{ background: '#000', border: '1px solid #333', padding: 12, fontSize: 14, lineHeight: 1.8, whiteSpace: 'pre-wrap', marginBottom: 10, color: '#ccc', maxHeight: 300, overflowY: 'auto' }}>
            {shareText}
          </div>
          <button onClick={() => { navigator.clipboard?.writeText(shareText); alert('Copiado!'); }}
            style={{ width: '100%', padding: 10, fontFamily: 'Space Mono,monospace', fontSize: 14, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', border: '1px solid #fff', background: '#fff', color: '#000' }}>
            Copiar Texto
          </button>
